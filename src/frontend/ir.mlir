module {
  func.func @rostin(%arg0: !emitc.ptr<!emitc.opaque<"FHEcontext">>, %arg1: !emitc.opaque<"FHEdouble">, %arg2: !emitc.opaque<"FHEdouble">, %arg3: !emitc.opaque<"FHEdouble">) -> !emitc.opaque<"FHEdouble"> attributes {llvm.linkage = #llvm.linkage<external>} {
    ...
    %4 = "emitc.constant"() <{value = #emitc.opaque<"std::vector<double>{0, 0, 0, 0, 0}">}> : () -> !emitc.opaque<"std::vector<double>">
    ...
    %6 = scf.for %arg4 = %c0 to %c5 step %c5_0 iter_args(%arg5 = %5) -> (!emitc.opaque<"FHEdouble">) {
      %8 = "emitc.constant"() <{value = #emitc.opaque<"std::vector<double>{0, 0, 0, 0, 0}">}> : () -> !emitc.opaque<"std::vector<double>">
      %9 = emitc.call "FHEencrypt"(%arg0, %8) : (!emitc.ptr<!emitc.opaque<"FHEcontext">>, !emitc.opaque<"std::vector<double>">) -> !emitc.opaque<"FHEdouble">
      %10 = "emitc.constant"() <{value = #emitc.opaque<"std::vector<double>{1, 1, 1, 1, 1}">}> : () -> !emitc.opaque<"std::vector<double>">
